...
public class Common {
    ...
    @Autowired
    public DbDao dao;
    public void checkuser(ModelMap m){
    ...
    @GetMapping("home-page?kw={kw}")
    public String Search(ModelMap m,@RequestParam(value = "kw",required = false,defaultValue = "") String kw){
        checkuser(m);
        response.setContentType("text/html;charset=UTF-8");
        m.addAttribute("productSearch",dao.getDataProductSearchbyName(kw));
        return "test";
    }
